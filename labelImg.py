...
from libs.labelshortcut import LabelShortcut #mango added
...
class MainWindow(QMainWindow, WindowMixin):
	...
	def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
		...
		self.labelShortcut=LabelShortcut(self.default_label_combo_box.items)
	...
	def keyReleaseEvent(self, event):
		...
		if event.key()>=48 and event.key()<=57:
			try:
				self.labelShortcut.onPress(self.current_item(),event.key())
				self.set_dirty()
				self.update_combo_box()
			except:
				print("none item")
			# self.labelShortcut.onPress(event.key())
		# if event.key()>=48 and event.key()<=57:
		# 	self.labelShortcut.onPress(event.key())
